...
if (process.env.PROD) {
    ...
    global.__mask_bin = require("path").join(__dirname, "..", "bin").replace(/\\/g, "\\\\")
} else {
    global.__mask_bin = require("path").join(process.cwd(), "bin").replace(/\\/g, "\\\\")
}
            ...
            portscanner.checkPortStatus(config.port, "127.0.0.1", (error, status) => {
                if (status == "closed") {
                    ...
                    dialog.showMessageBox(mainWindow, {
                        title: "Startup error",
                        message: `Mask Wallet is already open, or port ${config.port} is in use`,
                        type: "error",
                        buttons: ["ok"]
                    }, () => {
                        showConfirmClose = false
