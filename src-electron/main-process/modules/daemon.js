    ...
    checkVersion() {
        return new Promise((resolve, reject) => {
            if (process.platform === "win32") {
                let maskd_path = path.join(__mask_bin, "maskd.exe")
                let maskd_version_cmd = `"${maskd_path}" --version`
                if (!fs.existsSync(maskd_path))
                    resolve(false)
                child_process.exec(maskd_version_cmd, (error, stdout, stderr) => {
                    if(error)
                        resolve(false)
                    resolve(stdout)
                })
            } else {
                let maskd_path = path.join(__mask_bin, "maskd")
                let maskd_version_cmd = `"${maskd_path}" --version`
                if (!fs.existsSync(maskd_path))
                    resolve(false)
                child_process.exec(maskd_version_cmd, {detached: true}, (error, stdout, stderr) => {
                    if(error)
                        resolve(false)
                    resolve(stdout)
                })
    ...
    start(options) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if(options.app.testnet) {
                this.testnet = true
                args.push("--testnet")
                args.push("--log-file", path.join(options.app.data_dir, "testnet", "logs", "maskd.log"))
            } else {
                args.push("--log-file", path.join(options.app.data_dir, "logs", "maskd.log"))
            }
            ...
            if (process.platform === "win32") {
                this.daemonProcess = child_process.spawn(path.join(__mask_bin, "maskd.exe"), args)
            } else {
                this.daemonProcess = child_process.spawn(path.join(__mask_bin, "maskd"), args, {
                    detached: true
                })
            }
