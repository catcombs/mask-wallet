...
export class Backend {
    ...
    init(config) {
	    ...
	    this.config_dir = "C:\\ProgramData\\mask";
	    //this.config_dir = path.join(os.homedir(), "mask");
        } else {
            this.config_dir = path.join(os.homedir(), ".mask");
        }
    ...
    handle(data) {
        ...
        switch (data.method) {
            ...
            case "open_explorer":
                if(params.type == "tx") {
                    require("electron").shell.openExternal("https://18.232.142.59:8081/tx/"+params.id)
                }
                break;
