...
export class WalletRPC {
        ...
        return new Promise((resolve, reject) => {
            ...
            crypto.randomBytes(64+64+32, (err, buffer) => {
                ...
                if (process.platform === "win32") {
                    this.walletRPCProcess = child_process.spawn(path.join(__mask_bin, "mask-wallet-rpc.exe"), args)
                } else {
                    this.walletRPCProcess = child_process.spawn(path.join(__mask_bin, "mask-wallet-rpc"), args, {
                        detached: true
                    })
                }
    ...
    listWallets(legacy=false) {
        ...
        if(legacy) {
	        ...
	        legacy_paths = ["C:\\ProgramData\\MaskGUIWallet", "C:\\ProgramData\\MaskLITEWallet"]
            } else {
	        legacy_paths = [path.join(os.homedir(), "MaskGUIWallet"), path.join(os.homedir(), "MaskLITEWallet")]
            }
